...
def saveImage(data, res):
    ...
    df4 = df[df['labels'] == 3]
    df5 = df[df['labels'] == 4]
    bplot1 = plt.boxplot(df1.values[:, :4], patch_artist=True, labels=labels,
                         positions=(1, 1.3, 1.6, 1.9), widths=0.3)
    for patch, color in zip(bplot1['boxes'], colors):
        patch.set_facecolor(color)

    bplot2 = plt.boxplot(df2.values[:, :4], patch_artist=True, labels=labels,
    ...
    bplot3 = plt.boxplot(df3.values[:, :4], patch_artist=True, labels=labels,
                         positions=(4, 4.3, 4.6, 4.9), widths=0.3)

    for patch, color in zip(bplot3['boxes'], colors):
        patch.set_facecolor(color)

    bplot4 = plt.boxplot(df4.values[:, :4], patch_artist=True, labels=labels,
    ...
    bplot5 = plt.boxplot(df5.values[:, :4], patch_artist=True, labels=labels,
    ...
    plt.xlabel('聚类类别')
    plt.ylabel('归一化数据')
    plt.savefig('../../img/带有异常值箱型图/按类别的归一化数据箱型图.png')
    plt.show()
